...
(async () => {
  ...
  await page.goto("https://www.doubao.com/chat/", { waitUntil: "domcontentloaded", timeout: 20000 });
  await sleep(5000);
  ...
  const initialCount = await page.evaluate(() => {
    return Array.from(document.querySelectorAll('a[href*="/chat/"]'))
      .filter(a => a.href.match(/\/chat\/\d{10,}/)).length;
  });
  ...
  const clicked = await page.evaluate(() => {
    ...
    const els = Array.from(document.querySelectorAll("*")).filter(el =>
      el.textContent.trim() === "历史对话" && el.children.length <= 2
    );
    if (els.length > 0) {
      els[0].click();
      return "clicked 历史对话";
  ...
  for (let i = 0; i < maxAttempts; i++) {
    const count = await page.evaluate(() => {
      ...
      for (const el of all) {
        if (el.scrollHeight > el.clientHeight + 50 && el.clientHeight > 200 && el.clientHeight < 800) {
  ...
  let existingIndex = [];
  try { existingIndex = JSON.parse(fs.readFileSync(indexPath, "utf-8")); } catch(_) {}
  ...
  for (let i = 0; i < convs.length; i++) {
    ...
    process.stdout.write("[" + (i+1) + "/" + convs.length + "] " + conv.title.slice(0,40) + "... ");
    ...
    try {
      ...
      const msgs = await page.evaluate(() => {
        const results = [];
        const sels = ['[data-testid*="message"]', '[class*="message-item"]', '[class*="MessageItem"]',
          '[class*="chat-message"]', '[class*="turn-"]', '[role="article"]', 'article'];
        let els = [];
        for (const s of sels) {
          const found = Array.from(document.querySelectorAll(s)).filter(el => el.textContent.trim().length > 5);
          if (found.length >= 2) { els = found; break; }
        }
        if (els.length === 0) {
          const main = document.querySelector("main, [role='main'], [class*='chat-content'], [class*='ChatContent']");
          if (main) els = Array.from(main.querySelectorAll(":scope > div")).filter(d => d.textContent.trim().length > 10);
        }
        els.forEach((el, idx) => {
          ...
          if (dr) role = dr.includes("user") || dr.includes("human") ? "user" : dr.includes("assistant") || dr.includes("bot") ? "assistant" : "unknown";
          else if (cls.includes("user") || cls.includes("human") || cls.includes("question")) role = "user";
          else if (cls.includes("assistant") || cls.includes("bot") || cls.includes("answer") || cls.includes("response")) role = "assistant";
          else role = idx % 2 === 0 ? "user" : "assistant";
          const timeEl = el.querySelector("time") || el.querySelector("[class*='time']");
          results.push({ role, content: text.slice(0, 50000), timestamp: timeEl?.getAttribute("datetime") || "" });
        });
        return results;
      });

      const valid = msgs.filter(m => m.role !== "unknown" && m.content);
      if (valid.length === 0) { console.log("(0 msgs)"); continue; }

      const lines = valid.map(m => JSON.stringify({
        role: m.role, content: m.content,
        timestamp: m.timestamp || new Date().toISOString(),
        content_type: "text", message_id: "",
      }));
      fs.writeFileSync(path.join(OUTPUT_DIR, convId + ".jsonl"), lines.join("\n") + "\n");
      index.push({ id: convId, title: conv.title, message_count: valid.length, platform: "doubao" });
      totalNew++;
  ...
  console.log("\n" + "=".repeat(60));
  console.log("Done! Total:", index.length, "conversations (" + totalNew + " new,", totalMsgs, "new msgs)");
  await browser.close();
