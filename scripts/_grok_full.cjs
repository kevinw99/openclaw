...
(async () => {
  ...
  } catch(_) { console.log("No 查看全部 button"); }
  ...
  for (let i = 0; i < convs.length; i++) {
    ...
    try {
      ...
      const msgs = await page.evaluate(() => {
        const results = [];
        const sels = ['[data-testid*="message"]', '[class*="message"]', '[role="article"]', 'article'];
        let els = [];
        for (const s of sels) {
        ...
        if (els.length === 0) {
          ...
          if (main) els = Array.from(main.querySelectorAll(":scope > div > div")).filter(d => d.textContent.trim().length > 10);
        }
        els.forEach((el, idx) => {
          ...
          const dr = el.getAttribute("data-role") || el.getAttribute("data-message-author-role") || "";
          if (dr) role = dr.includes("user") ? "user" : dr.includes("assistant") || dr.includes("grok") ? "assistant" : "unknown";
          else if (cls.includes("user") || cls.includes("human")) role = "user";
          else if (cls.includes("assistant") || cls.includes("bot") || cls.includes("grok") || cls.includes("model")) role = "assistant";
