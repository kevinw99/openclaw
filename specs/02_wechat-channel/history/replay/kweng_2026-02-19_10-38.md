...
> Person: kweng | Messages: 142 | Turns: 7
...
Before the learning plan — **spec 01 is substantially built already**. Swabble exists as a complete Swift package at `/Swabble/` with ~40 files covering everything in the spec. This changes what you need to learn.
...
```
Mic → AVAudioEngine → SpeechAnalyzer → SpeechTranscriber → WakeWordGate → HookExecutor
```
...
You only need to understand **3 layers** of openclaw — ignore the rest (TypeScript server, channels, gateway, extensions).

### Layer 1: Swabble's own architecture (2 hours) — **must know**
...
| `Swabble/Sources/SwabbleCore/Config/Config.swift`         | The Codable config structure — everything flows from this |
| `Swabble/Sources/SwabbleCore/Speech/SpeechPipeline.swift` | Core actor: mic → transcription stream                    |
...
| `Swabble/Sources/swabble/Commands/ServeCommand.swift`     | The main loop — ties everything together                  |

**Mental model:** `ServeCommand` starts `SpeechPipeline` → iterates `AsyncStream<SpeechSegment>` → checks `WakeWordGate` → fires `HookExecutor`. That's the whole daemon.

### Layer 2: macOS app integration (30 min) — **only if you're integrating with the GUI app**
...
### Layer 3: openclaw conventions (15 min) — **for consistency only**

Read just two things:

- `AGENTS.md` — skim the Swift section for naming conventions (`@Observable`, StrictConcurrency, `@MainActor`)
- The Commander CLI pattern: look at `apps/macos/Sources/OpenClawMacCLI/` (3 command files) — same pattern Swabble uses

---

## What You Can Ignore Entirely

- `src/` — TypeScript/Node.js server (irrelevant to Swabble)
- `extensions/` — Telegram, Slack, etc. channel plugins
- `docs/` — End-user docs for openclaw server
- `apps/ios/`, `apps/android/` — Not related
- `packages/`, `ui/`, `vendor/` — Server-side JS infrastructure

---

## Suggested Next Steps

1. **Build Swabble first** — run `swift build` in `Swabble/` and see what compiles on macOS 26
2. **Fill the gaps** — the UNIX socket for `status`, launchd `start/stop/restart`
3. **Wire segment timing** — plumb speech segment timing from `SpeechPipeline` through to the `WakeWordGate` in CLI (the spec calls this out as incomplete)

The openclaw repo itself is mostly a distraction for your goal. The relevant learning is 90% within `Swabble/` itself.

_Tools: Glob (4), Read (4), Task (1) -- /Users/kweng/AI/openclaw/Swabble/Sources/SwabbleCore/Speech/SpeechPipeline.swift, /Users/kweng/AI/openclaw/Swabble/Sources/swabble/Commands/ServeCommand.swift, /Users/kweng/AI/openclaw/Swabble/docs/spec.md, /Users/kweng/AI/openclaw/requests-claw.txt, pattern:\*\*/_.md +3 more\*
...
_Tools: Bash (1) -- Print working directory_
...
**Result:**
My mistake from earlier — **Swabble is not spec 01**. Let me correct this clearly.

---

## Clarification

**Spec 01** is at `spec/01_full-context-ai-assistant/` — a **Full Context AI Personal Assistant** built on top of OpenClaw. Nothing to do with Swift or mobile apps.

**Swabble** (`Swabble/`) is a completely separate thing — a macOS voice daemon (Swift) that's part of the openclaw ecosystem for voice input. It's irrelevant to your goal. Ignore it.
...
This is now a very different learning path — no Swift, no mobile, just understanding OpenClaw as an AI agent platform.
...
| Workspace files   | `docs/concepts/`          | AGENTS.md, MEMORY.md, SOUL.md, USER.md — these define the agent's identity and context |
...
1. **USER.md** — structured profile (name, preferences, history, projects, work context)
2. **Project directories** — `workspace/projects/<name>/` with README, status, notes, decisions
3. **HEARTBEAT.md** — proactive check-in behavior
4. **MEMORY.md** — curated long-term context about you
5. **Custom skills** — memory consolidation, project templates, feedback capture

### Layer 3: What you can skip entirely

- Swift / Swabble — irrelevant
- Channel integration code (`extensions/`) — already works, just configure
- Server infrastructure (`src/`) — don't need to touch it
- `apps/` (iOS/Android/macOS) — optional, not core to spec 01
...
_Tools: Bash (4), Read (2) -- /Users/kweng/AI/openclaw/spec/01_full-context-ai-assistant/README.md, /Users/kweng/AI/openclaw/spec/01_full-context-ai-assistant/requirements.md -- Find spec and numbered files in repo; List top-level directory; List spec 01 directory_
...
| ---------------------------- | ---------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| WeChat Web protocol (old)    | [itchat](https://pypi.org/project/itchat/) (Python)              | **Dead** — Tencent shut down web.wechat.com for personal accounts in 2021                                        |
| RPA/puppet over official app | [Wechaty](https://github.com/wechaty/wechaty) (Node.js)          | **Alive but costly** — free tier uses a puppet that requires a paid subscription (~$15/mo) for personal accounts |
| Windows PC UI automation     | [wxauto](https://github.com/cluic/wxauto) (Python, Windows-only) | **Working but fragile** — scrapes the Windows WeChat client UI; breaks on WeChat updates                         |
| Protocol reverse engineering | Various                                                          | **High ban risk** — account termination likely                                                                   |
...
- `WR5` Voice message receive (extremely common in China — transcribe to text)
...
### WeChat-Specific: Moments (朋友圈)

- `WM1` Read Moments feed — retrieve recent posts from all contacts
...
- `WM8` Moments visibility scope (public / friends only / specific people) — read-only awareness

### WeChat-Specific: Channels (视频号) _(lower priority)_

- `WC1` Read subscribed channel posts (short videos/live streams)
- `WC2` Detect when a followed creator goes live

### WeChat-Specific: Official Accounts (公众号) _(for context awareness)_
...
- `WI1` Maintain contact list with display names, remarks (备注), and tags
...
| --------------------- | ---------------------------- | -------------------------------------------- |
| Library maturity      | Baileys — stable, free, open | Wechaty — works but paid puppet for personal |
...
| Voice messages        | Transcription optional       | Critical (must have — very common in China)  |
| Implementation effort | Already done                 | ~3-4x harder than WhatsApp                   |
| Platform              | Any OS                       | Wechaty: any; wxauto: Windows only           |

**Bottom line**: WeChat integration is buildable using Wechaty (same TypeScript/Node.js pattern as openclaw's other extensions), but Moments reading is the hardest part — most puppet implementations don't expose it cleanly. It would likely require a dedicated Windows or Android bridge to read Moments reliably.
...
_Tools: Bash (3), Grep (1), Read (1), WebSearch (3) -- /Users/kweng/AI/openclaw, /Users/kweng/AI/openclaw/docs/channels/whatsapp.md, pattern:wechat|WeChat|we_chat -- List available channel extensions; Look at zalo extension structure as a comparable channel; Look at zalo extension internals_
...
├── package.json                # deps: wechaty, wechaty-puppet-padlocal, wechaty-puppet-wechat4u
├── index.ts                    # Plugin entry: register channel + http handler
└── src/
    ├── config-schema.ts        # Zod schema for ~/.openclaw/openclaw.json wechat section
    ├── accounts.ts             # Resolve account config (no API token — session file based)
    ├── channel.ts              # ChannelPlugin + ChannelDock (capabilities, routing, policy)
    ├── runtime.ts              # PluginRuntime singleton (same pattern as Zalo)
    ├── bot.ts                  # Wechaty bot lifecycle: create, start, stop, singleton per account
    ├── monitor.ts              # Event dispatcher: bot.on('message') → processMessage pipeline
    ├── send.ts                 # Outbound: bot.Contact.find() → say(), Room.find() → say()
    ├── actions.ts              # WeChat tool actions: react (emoji), forward, recall
    ├── onboarding.ts           # QR login flow: scan event → serve QR as PNG or terminal art
    ├── probe.ts                # Health check: bot.logonoff() + contact self info
    ├── status-issues.ts        # Detect: not logged in, puppet error, disconnected
    ├── types.ts                # WeChatMessage, WeChatContact, WeChatRoom types
    ├── voice.ts                # Voice msg: toFileBox() → save → transcribe (whisper or OS)
    ├── moments.ts              # Moments feed: padlocal only — poll + parse → context events
    └── contact-graph.ts        # Build relationship index from contacts + group memberships
```
...
| Concern          | Zalo                        | WeChat                                                    |
| ---------------- | --------------------------- | --------------------------------------------------------- |
| Auth             | Bot API token (server-side) | QR scan → session file (client-side)                      |
...
export async function createWechatyBot(account: ResolvedWeChatAccount): Promise<WechatyInstance> {
  ...
  const bot = WechatyBuilder.build({
    name: `openclaw-${account.accountId}`, // → session file path
    puppet,
    puppetOptions: account.puppet === "padlocal" ? { token: account.padlocalToken } : {},
  });
  return bot;
}
```

#### `monitor.ts` — Event Dispatcher (replaces polling loop)
...
#### `monitor.ts` — `processMessage` flow

```
msg.self() → skip (own messages)
msg.type() → dispatch:
  Text     → processTextMessage()
  Audio    → voice.transcribe() → processTextMessage()
  Image    → saveMedia() → processMediaMessage()
  Video    → saveMedia() → processMediaMessage()
  Recalled → log recall event
  Contact  → extract contact card → processTextMessage("<contact: Name>")
  Url      → extract URL + title → processTextMessage("<link: title url>")

processTextMessage():
  msg.room() → isGroup = true, chatId = room.id, roomTopic = await room.topic()
  msg.from() → senderId = contact.id, senderName = await contact.name()
  isGroup && requireMention && !await msg.mentionSelf() → skip

  → dmPolicy check (same pattern as Zalo)
  → pairing flow if unknown sender
  → core.channel.routing.resolveAgentRoute(...)
  → core.channel.reply.formatAgentEnvelope(...)  [channel: "WeChat", from: senderName, ...]
  → core.channel.reply.dispatchReplyWithBufferedBlockDispatcher(...)
      deliver: → send.ts → contact.say(text) or room.say(text)
```

#### `moments.ts` — Moments Feed (padlocal only)

```typescript
// Moments is NOT in Wechaty's standard API — padlocal exposes it via a custom API
...
#### `contact-graph.ts` — Relationship Index
...
export type ContactNode = {
  ...
  remark: string; // your custom remark (备注)
...
openclaw channels login --channel wechat [--account personal]
  → creates WechatyBot with puppet
  → bot.on('scan') → prints QR to terminal (or serves PNG at /wechat/qr)
  → user scans with WeChat app
  → bot.on('login') → session saved to ~/.openclaw/credentials/wechat/<accountId>/session.json
  → session auto-restores on gateway restart (Wechaty profile persistence)
```
...
**Phase 1 — Core messaging** (parity with other channels)
...
- `voice.ts` — voice message transcription (critical for China usage)
- Image/video/file receive, media download pipeline

**Phase 3 — WeChat-specific**

- `moments.ts` — Moments feed polling (padlocal only)
- `contact-graph.ts` — relationship indexing
- `actions.ts` — emoji reactions, contact card sending

**Phase 4 — Spec 01 context**

- Moments → injected as session context for the AI
- Contact graph → available as tool (`wechat_contacts`, `wechat_moments`)
...
_Tools: Read (5), WebFetch (4), WebSearch (1) -- /Users/kweng/AI/openclaw/extensions/zalo/index.ts, /Users/kweng/AI/openclaw/extensions/zalo/openclaw.plugin.json, /Users/kweng/AI/openclaw/extensions/zalo/src/channel.ts, /Users/kweng/AI/openclaw/extensions/zalo/src/monitor.ts, /Users/kweng/AI/openclaw/extensions/zalo/src/runtime.ts_
...
**`README.md`** — Overview, scope boundaries, key decisions, references to spec 01 as the consumer

**`requirements.md`** — 13 functional requirements (FR1–FR13) covering:
...
**`tasks.md`** — 19 tasks across 5 phases with dependency graph:

- Phase 1: Scaffolding (4 tasks)
- Phase 2: Bot lifecycle (4 tasks)
- Phase 3: Message pipeline (5 tasks) — critical path
- Phase 4: WeChat features — voice, Moments, contact graph (3 tasks)
- Phase 5: Docs & tests (3 tasks)

**`status.md`** — Tracking board, decisions log, blockers

Want to start on Phase 1 implementation now?

_Tools: Bash (3), Read (4), Write (5) -- /Users/kweng/AI/openclaw/spec/01_full-context-ai-assistant/design.md, /Users/kweng/AI/openclaw/spec/01_full-context-ai-assistant/tasks.md, /Users/kweng/AI/openclaw/spec/02_wechat-channel/README.md, /Users/kweng/AI/openclaw/spec/02_wechat-channel/design.md, /Users/kweng/AI/openclaw/spec/02_wechat-channel/requirements.md +4 more -- List template directory; Create spec directory; Verify all spec files created_
